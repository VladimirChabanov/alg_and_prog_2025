...
13. Установите набор расширений (плагинов) "Remote Development";
...
23. Переключитесь в раздел *Explorer* ![](./practice_01_img/еxprorer.png) и выберите `"main.cpp"`.
...
66. Повторно подключитесь к серверу через VS Code, откройте в редакторе каталог `"~/projects/cpp"` и откройте проект в контейнере.  
...
85. Нажмите сочетание клавиш <kbd>Shift</kbd> + <kbd>F11</kbd> (*step out*) два раза и остановите отладку.  
...
31. Снова подключитесь к серверу. 
...
38. Откройте терминал и команду, затем переключитесь на вкладку *Explorer*:
    ```bash
    go mod init hello
    ```
