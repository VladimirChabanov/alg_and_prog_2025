...
- за практические работы: $Б_{ПР} \in [0..8]$;
- за решение задач: $Б_{задачи} \in [0..14]$;
- за групповой проект: $Б_{проект} \in [0..16]$;
...
$$Б_{тесты} = 12 \cdot \frac{1}{6} \sum_{i=1}^{6}\frac{Б_{тест\ i}}{100}$$
...
$$Б_{ПР} = 8 \cdot \frac{1}{2} \sum_{i=1}^{2}\frac{Б_{ПР\ i}}{100}$$

Оценка за первую практическую работу выставляется как оценка за отчёт. Оценка за вторую практическую работу выставляется как произведение оценки за отчёт и за защиту:

$$Б_{ПР2} = Б_{ПР2\ отчёт}\cdot\frac{Б_{ПР2\ защита}}{100}$$

### Задачи

Каждый блок задач оценивается отдельно по 100-балльной шкале и вносит одинаковый вклад в оценку за этот вид деятельности:

$$Б_{задачи} = 14 \cdot \frac{1}{9} \sum_{i=1}^{9}\frac{Б_{задачи\ i}}{100}$$

Оценка за блок задач выставляется как произведение процента решённых задач на оценку полученную во время защиты этого блока. Оценка за первый блок выставляется просто как процент решённых задач:

$$Б_{задачи\ 1} = \frac{N_{решено}}{N_{задач\ в\ блоке}}$$

$$Б_{задачи\ i} = \frac{N_{решено}}{N_{задач\ в\ блоке}} \cdot \frac{Б_{защита}}{100}$$
...
$$Б_{проект} = 16 \cdot \frac{проект}{100}$$
...
Тестирование состоит из 100 вопросов по 1 баллу за каждый вопрос. На тестирование отводится 1 час 30 минут. Пробный [тест для подготовки](https://moodle.cfuv.ru/mod/quiz/view.php?id=757147).
...
Каждая задача состоит из 3х последовательных шагов:

- **Чтение и обход элементов матрицы**

  - Считать элементы матрицы со стандартного ввода.
  - Выполнить итерацию по элементам согласно заданному алгоритму обхода. Примеры алгоритмов:
    - движение от элемента с индексом [*i*,*j*] против часовой стрелки;
    - обход от правого нижнего угла по диагоналям (для чётных диагоналей — снизу вверх, для нечётных — сверху вниз) и др.

- **Фильтрация, трансформация или поиск в последовательности элементов**

  Применить к ряду элементов, полученному на предыдущем шаге, одну из операций. Примеры операций:

  - отбор элементов на чётных позициях, взаимно простых с числом 19;
  - преобразование элементов: деление на 2 тех, что превышают среднее значение в своём столбце; прибавление к элементам, меньшим среднего в столбце, значения, полученного для предыдущего рассматриваемого элемента, и т. п.

- **Трансформация исходной матрицы**

  - Использовать набор элементов, сформированный на втором этапе, для изменения исходной матрицы. Примеры действий:
    - сложить элементы полученного ряда в матрицу той же размерности, что и исходная (заполняя её начиная с правого верхнего угла), затем прибавить эту матрицу к исходной;
    - обнулить в исходной матрице все значения, являющиеся делителями хотя бы одного из элементов ряда, полученного на предыдущем шаге.
